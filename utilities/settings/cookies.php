<?php 
    ...
    function setCookieWithAttributes($name, $value, $domain, $path, $expires, $samesite, $secure) {
    ...
    foreach ($crossSiteCookies as $name => $cookieData) {
        setCookieWithAttributes($name, $cookieData[0], $cookieData[1], '/', time() + 3600, 'None', true);
    }

    // Set SameSite=Strict for cookies not sent in cross-site requests
    foreach ($nonCrossSiteCookies as $name => $cookieData) {
        setCookieWithAttributes($name, $cookieData[0], $cookieData[1], '/', time() + 3600, 'Strict', false);
    }*/
